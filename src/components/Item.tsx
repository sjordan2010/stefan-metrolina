        ...
        >
          {isExpanded &&
            item.locations.map((location, index) => {
              return (
                <div
                  className="w-full flex items-center gap-4 lg:gap-10 py-2 mb-2 lg:text-base lg:px-10"
                  key={location.locationId + index}
                >
                  <MapPinIcon width={20} className="text-green-600" />
                  <p>
                    <span className="italic">Location ID: </span> {location.locationId}
                  </p>
                  <p>
                    <span className="italic">Quantity on Hand: </span> {location.onHandQty}
                  </p>
                </div>
              );
            })}
